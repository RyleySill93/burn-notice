...
COMMENT ON COLUMN audit.entry.app_impersonator_id IS 'Pk of user who is impersonating provided user id.';
...
BEGIN
 ...
 -- TG_RELID,                                               -- relation OID for much quicker searches
    ...
    NULL,                                                   -- changed_data
    'f',                                                    -- statement_only
    NULL                                                    -- application impersonator id
  );
  ...
  -- Set impersonator_id if not null
  IF CURRENT_SETTING('auditcontext.impersonator_id', True) IS NOT NULL THEN
    audit_row.app_impersonator_id = CURRENT_SETTING('auditcontext.impersonator_id');
  END IF;
  ...
  IF TO_REGCLASS(audit_table_name) IS NULL THEN
    -- This code should only run at the end of every month!
    -- Generate a unique lock identifier
    lock_id := ('x' || substr(md5(audit_table_name), 1, 16))::bit(64)::bigint;
    -- Wait to acquire a lock avoiding concurrent creation requests
    PERFORM pg_advisory_xact_lock(lock_id);
    -- Double-check if the partition still does not exist after acquiring the lock
    IF TO_REGCLASS(audit_table_name) IS NULL THEN
      -- The lock was acquired and the partition does not exist, so create it
    ...
    END IF;
    -- The lock will be automatically released at the end of this transaction
