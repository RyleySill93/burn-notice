...
class HTTPAppContextMiddleware(BaseHTTPMiddleware):
    """
    Sets application context for downstream auditors and logging
    """

    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
        token = context.initialize(
            user_type=context.AppContextUserType.USER.value,
            breadcrumb=request.url.path,
        )
...
class DramatiqAuditMiddleware(Middleware):
    """
    Set the application context to system to indicate this is an
    actor potentially performing database changes
    """
    ...
    def before_process_message(self, broker, message):
        context.initialize(
            ...
            user_id='user-system',
            breadcrumb=f'{message.actor_name}',
        )
