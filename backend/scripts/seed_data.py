...
def _run_seed(db):
    ...
    for eng_data in ENGINEERS:
        ...
        if not membership:
            membership = Membership.create(MembershipCreate(
        ...
        if not engineer:
            engineer = Engineer.create(EngineerCreate(
                customer_id=customer.id,
        ...
        for engineer in engineers_created:
            ...
            for _ in range(num_sessions):
                ...
                for call_idx in range(num_calls):
                    ...
                    usage = Usage.create(UsageCreate(
                    ...
                    db.session.execute(
                        Usage.__table__.update()
                        .where(Usage.__table__.c.id == usage.id)
            ...
            if days_ago > 0 and daily_tokens > 0:
                UsageDaily.create(UsageDailyCreate(
                    engineer_id=engineer.id,
                    date=current_date,
